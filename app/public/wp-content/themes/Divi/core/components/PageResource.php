...
class ET_Core_PageResource {
	 ...
	 */
	public static function remove_static_resources( $post_id, $owner = 'core', $force = false, $slug = 'all', $preserve_vb_files = false ) {
		...
		self::do_remove_static_resources( $post_id, $owner, $force, $slug, $preserve_vb_files );
	}
	 ...
	 * @param bool   $preserve_vb_files Whether to preserve files containing "-vb-" string. Default false.
	 */
	public static function do_remove_static_resources( $post_id, $owner = 'core', $force = false, $slug = 'all', $preserve_vb_files = false ) {
		...
		self::_remove_files_in_directory( $files, $cache_dir, $preserve_vb_files );
	 ...
	 * @param bool    $preserve_vb_files Whether to preserve files containing "-vb-" string. Default false.
	 */
	protected static function _remove_files_in_directory( $files, $cache_dir, $preserve_vb_files = false ) {
		foreach ( $files as $file ) {
			...
			// Skip Visual Builder files if preservation is enabled.
			if ( $preserve_vb_files && strpos( basename( $file ), '-vb-' ) !== false ) {
				continue;
			}
