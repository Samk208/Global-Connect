...
class StaticCSS implements DependencyInterface {
	...
	public static function setup_styles_manager( int $post_id = 0 ) {
		...
		// Include pagination and random order state in cache key to prevent cross-request caching issues.
		$cache_key = $post_id . intval( $should_generate_critical_css ) . intval( $has_paginated_loops ) . intval( $current_page_has_random_order_loops );
		...
		$forced_inline = ! $post_id || $is_preview || $forced_in_footer || $static_css_is_disabled || $has_paginated_loops || $current_page_has_random_order_loops || et_core_is_safe_mode_active() || ET_GB_Block_Layout::is_layout_block_preview();
