	...
	public static function sanitize_css_properties( string $css ): string {
		...
		$used_properties = array_filter(
			...
			function ( $allow ) use ( $css ) {
				...
				return (bool) preg_match( '/' . $allow . '\s*?:(.+)/i', $css );
			}
		);
		 ...
		 *      -- Break the loop.
		 */
		while ( '' !== $css && true === (bool) preg_match( $regex_pattern, "{$css}{$used_properties[0]}:", $matches ) ) {
		...
		foreach ( $extracted_properties as $item ) {
			...
			if ( in_array( $property, [ 'filter', 'backdrop-filter' ], true ) && $css_filter_functions ) {
