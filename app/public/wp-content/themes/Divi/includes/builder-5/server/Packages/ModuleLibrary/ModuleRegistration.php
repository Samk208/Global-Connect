	...
	public static function register_module( string $metadata_folder, array $args = [] ): ?WP_Block_Type {
		...
		if ( isset( $register_args['render_callback'] ) ) {
			...
			$register_args['render_callback'] = function ( $block_attributes, $content, WP_Block $block ) use ( $loop_wrapped_render_callback, $metadata ) {
				...
				// Check if module is disabled on all breakpoints and not used as interaction target.
				// If so, remove it from DOM completely (matching Divi 4 behavior).
				$disabled_on_attrs  = $module_attrs['module']['decoration']['disabledOn'] ?? [];
				$interaction_target = $module_attrs['module']['decoration']['interactionTarget'] ?? '';

				if ( self::_is_disabled_on_all_breakpoints( $disabled_on_attrs ) && empty( $interaction_target ) ) {
					return '';
				}
				...
				// If not displayable, return empty string without rendering.
