...
class DynamicAssets implements DependencyInterface {
	 ...
	 * @var string
	 */
	private $_original_post_content = '';
	...
	public function load() {
		...
		add_action( 'wp_enqueue_scripts', [ $this, 'enqueue_dynamic_assets' ] );
		add_action( 'wp_enqueue_scripts', [ $this, 'enqueue_dynamic_scripts_early' ] );

		// If this is the Divi theme, add the divi filter to the global assets list.
		if ( 'divi' === $shortname ) {
			add_filter( 'divi_frontend_assets_dynamic_assets_global_assets_list', [ $this, 'divi_get_global_assets_list' ] );
		}
		...
		add_action( 'wp_footer', [ $this, 'enqueue_dynamic_scripts_late' ] );

		add_action( 'wp_footer', [ $this, 'wp_footer' ] );

		// Add script that loads fallback .css during blog module ajax pagination.
		add_action( 'wp_footer', [ $this, 'maybe_inject_fallback_dynamic_assets' ] );
		// If a late file was generated, we grab it in the footer and then inject it into the header.
		add_action( 'divi_frontend_assets_dynamic_assets_utils_late_assets_generated', [ $this, 'maybe_inject_late_dynamic_assets' ], 0 );
	...
	public function pre_initial_setup() {
		...
		$this->_original_post_content = $current_post ? $current_post->post_content : '';

		// Set some Dynamic Assets class base properties.
		$_page_content = $content_retriever->get_entire_page_content( $current_post );
		$_page_content = $this->maybe_add_global_modules_content( $_page_content );
		$_page_content = $this->maybe_add_appended_canvas_content( $_page_content, $current_post_id );
		$this->_set_all_content( $_page_content );
	...
	public function get_assets_data( array $asset_list = [] ): array {
		...
		$no_protocol_path      = str_replace( [ 'http:', 'https:' ], '', $this->_product_dir );
	...
	public function get_global_assets_list(): array {
		...
		if ( ! $this->_use_global_colors ) {
			...
			if ( $this->_post_id && is_singular() ) {
				// Get main post content to extract interaction target IDs.
				$main_content = $this->_original_post_content;

				if ( $main_content ) {
					// Get all appended canvas content.
					$appended_canvas_content = OffCanvasHooks::get_all_appended_canvas_content( $this->_post_id, $main_content );
			...
			$this->_early_global_asset_list['et_early_global_colors'] = [
		...
		if ( ! $this->_use_fa_icons && is_category() && ! empty( $this->_tb_template_ids ) ) {
			...
			if ( ! empty( $template_content ) ) {
				...
				} elseif ( strpos( $template_content, 'FontAwesome' ) !== false || strpos( $template_content, 'fa-' ) !== false || ( strpos( $template_content, 'unicode' ) !== false && strpos( $template_content, '"fa"' ) !== false ) || ( strpos( $template_content, 'type' ) !== false && strpos( $template_content, '"fa"' ) !== false ) ) {
		...
		if ( $this->_use_divi_icons ) {
			$this->_early_global_asset_list['et_icons_all'] = [
				'css' => "{$assets_prefix}/css/icons_all.css",
			];
		} elseif ( $this->_use_social_icons ) {
			$this->_early_global_asset_list['et_icons_social'] = [
				'css' => "{$assets_prefix}/css/icons_base_social.css",
			];
		} else {
			$this->_early_global_asset_list['et_icons_base'] = [
				'css' => "{$assets_prefix}/css/icons_base.css",
			];
		}

		if ( $this->_use_fa_icons ) {
			$this->_early_global_asset_list['et_icons_fa'] = [
		...
		if ( ( is_single() && ! $this->_page_builder_used ) || ( is_home() && ! is_front_page() ) || ! is_singular() ) {
			$this->_early_global_asset_list['et_post_formats'] = [
				'css' => [
		...
		if ( ! is_page() ) {
			$this->_early_global_asset_list['et_posts'] = [
				'css' => "{$assets_prefix}/css/posts{$this->_cpt_suffix}.css",
			];
		}

		if ( $this->is_rtl ) {
			$this->_early_global_asset_list['et_divi_shared_conditional_rtl'] = [
		...
		if ( $flex_used && $grid_items_used ) {
			$this->_early_global_asset_list['grid_items_flex'] = [
				'css' => [
		...
		if ( $block_used && $grid_items_used ) {
			$this->_early_global_asset_list['grid_items_block'] = [
				'css' => [
		...
		if ( is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
			$this->_early_global_asset_list['et_divi_woocommerce_modules'] = [
				'css' => [
		...
		if ( is_plugin_active( 'wp-pagenavi/wp-pagenavi.php' ) ) {
			$this->_early_global_asset_list['et_divi_wp_pagenavi'] = [
		...
		if ( $show_in_lightbox ) {
			$this->_early_global_asset_list['et_jquery_magnific_popup'] = [
		...
		if ( $has_animation_style || in_array( 'divi/circle-counter', $this->_processed_modules, true ) ) {
			$this->_early_global_asset_list['animations'] = [
		...
		if ( $has_block_mode_blog ) {
			$this->_early_global_asset_list['blog_block'] = [
		...
		if ( $sticky_used ) {
			$this->_early_global_asset_list['sticky'] = [
	...
	public function get_late_global_assets_list(): array {
		...
		if ( $this->_late_global_color_ids ) {
			...
			if ( ! empty( $late_global_color_ids ) ) {
				$this->_late_global_asset_list['et_late_global_colors'] = [
		...
		if ( $this->_late_custom_icon ) {
			$this->_late_global_asset_list['et_icons_all'] = [
				'css' => "{$assets_prefix}/css/icons_all.css",
			];
		} elseif ( $this->_late_social_icon ) {
			$this->_late_global_asset_list['et_icons_social'] = [
				'css' => "{$assets_prefix}/css/icons_base_social.css",
			];
		}

		if ( $this->_late_fa_icon ) {
			$this->_late_global_asset_list['et_icons_fa'] = [
				'css' => "{$assets_prefix}/css/icons_fa_all.css",
			];
		}
		...
		$gutter_widths = [];
		if ( ! empty( $this->_late_gutter_width ) ) {
		...
		if ( $flex_used && $grid_items_used ) {
			$this->_late_global_asset_list['grid_items_flex'] = [
				'css' => [
					"{$assets_prefix}/css/grid_items_flex{$this->_cpt_suffix}.css",
				],
			];
		...
		if ( $block_used && $grid_items_used ) {
			$this->_late_global_asset_list['grid_items_block'] = [
				'css' => [
					"{$assets_prefix}/css/grid_items{$this->_cpt_suffix}.css",
				],
			];
		...
		if ( $this->_late_show_in_lightbox ) {
			$this->_late_global_asset_list['et_jquery_magnific_popup'] = [
				'css' => "{$assets_prefix}/css/magnific_popup.css",
			];
		}

		if ( $this->_late_use_animation_style ) {
			$this->_late_global_asset_list['animations'] = [
				'css' => "{$assets_prefix}/css/animations{$this->_cpt_suffix}.css",
			];
		}

		if ( $this->_late_use_block_mode_blog ) {
			$this->_late_global_asset_list['blog_block'] = [
				'css' => "{$assets_prefix}/css/blog_block{$this->_cpt_suffix}.css",
			];
		}

		if ( $this->_late_use_sticky ) {
			$this->_late_global_asset_list['sticky'] = [
				'css' => "{$assets_prefix}/css/sticky_elements{$this->_cpt_suffix}.css",
			];
		}
	...
	public function enqueue_dynamic_scripts( string $request_type = 'early' ) {
		...
		if ( ! $this->_enqueue_audio ) {
			...
			$has_blog_module = $this->check_for_dependency( [ 'divi/blog' ], $current_modules );

			$this->_enqueue_audio = $this->check_post_format_dependency( 'audio' ) || $this->check_for_dependency( $audio_deps, $current_modules ) || $has_blog_module;
		...
		if ( 'late' === $request_type ) {
			...
			// Check both content detection and registered script data (for interactions from option group presets).
			if ( DetectFeature::has_interactions_enabled( $this->_get_all_content(), $this->_options ) ) {
				$this->_enqueue_interactions = true;
			}

			// Also check if interactions script data has been registered (e.g., from option group presets).
			// This ensures interactions from presets are detected even if not in the original content.
			$interactions_script_data = ScriptData::get_data( 'interactions' );
			if ( ! empty( $interactions_script_data ) ) {
				$this->_enqueue_interactions = true;
			}
	...
	public function check_if_attribute_exits( string $feature, string $content ): bool {
		...
		if ( ! empty( $this->_presets_feaure_used ) ) {
			...
			return isset( $this->_presets_feaure_used[ $feature ] ) && ! empty( $this->_presets_feaure_used[ $feature ] );
		}
	 ...
	 */
	public function check_if_class_exits( string $class_name, string $content ) {
		// Ensure the $content is valid string.
		$content = is_string( $content ) ? $content : '';

		return preg_match( '/class=".*' . preg_quote( $class_name, '/' ) . '/', $content );
	}
