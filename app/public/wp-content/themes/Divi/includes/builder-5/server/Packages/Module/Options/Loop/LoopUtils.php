...
class LoopUtils {
	 ...
	 * preventing memory exhaustion on large content. The method searches for "loop"
	 * position first, then extracts a bounded substring (50KB max) and applies a
	 * regex pattern with limited quantifier to detect "enable":"on" within that window.
	 *
	...
	public static function has_any_loop_enabled_blocks( $content ) {
		...
		// Quick check before expensive regex - bail early if loop pattern doesn't exist.
		$loop_pos = strpos( $content, '"loop"' );
		if ( false === $loop_pos ) {
			return false;
		}

		// Extract limited substring (50KB max) starting from "loop" position.
		...
		$window_size    = 51200; // 50KB limit.
		$substring      = substr( $content, $loop_pos, min( $window_size, $content_length - $loop_pos ) );

		// Check if content contains loop-enabled blocks using regex pattern with limited quantifier.
