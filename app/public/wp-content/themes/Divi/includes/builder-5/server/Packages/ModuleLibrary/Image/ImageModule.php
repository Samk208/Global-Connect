...
class ImageModule implements DependencyInterface {
	...
	public static function svg_style_declaration( $params ) {
		...
		// Width, height, minHeight, and maxHeight are already in attrValue for current breakpoint (no inheritance needed).
		...
		if ( $is_src_svg ) {
			...
			// Use user's width if set, otherwise fallback to auto.
			$style_declarations->add( 'width', '' !== $width ? $width : 'auto' );
